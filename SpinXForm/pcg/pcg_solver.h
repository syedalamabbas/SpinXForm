...
//    -BLAS routines are hard-coded rather than linked from an external library
//    -a diagonal preconditioner was added as a fallback
//    -sparse matrix-dense vector multiply was optimized
//
...
// inf-norm (maximum absolute value)
{
   ...
   for( std::vector<double>::const_iterator xi = x.begin(); xi != x.end(); xi++ )
   {
      norm = max( norm, *xi );
   }
...
template<class T>
void factor_diagonal(const SparseMatrix<T> &matrix, SparseColumnLowerFactor<T> &factor,
                     T modification_parameter=0.97, T min_diagonal_ratio=0.25)
{
   ...
   for( int i = 0; i < n; i++ )
   {
      factor.invdiag[i] = 1. / matrix( i, i );
      factor.rowindex.push_back( i );
      factor.colstart.push_back( i );
   }
   factor.colstart.push_back( n );
}
...
struct PCGSolver
{
   ...
   void set_solver_parameters(T tolerance_factor_, int max_iterations_, T modified_incomplete_cholesky_parameter_=0.97, T min_diagonal_ratio_=0.25, bool use_mic_=true)
   {
      use_mic = use_mic_;
   ...
   void form_preconditioner(const SparseMatrix<T>& matrix)
   {
      if(use_mic) {
         ...
         factor_diagonal(matrix, ic_factor);
      }
   }
